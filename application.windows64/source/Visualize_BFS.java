...
public class Visualize_BFS extends PApplet {
  ...
  if (mode == 1)
  {
   ...
   for(int i = 0; i <= rear; i++)
   {
     if (dist[q[i].r][q[i].c] <= curPos)
     {
       ...
       colorMode(HSB);
       int fillColor = lerpColor(color(0xffC62833),color(0xff2CFF0D), 1f*dist[q[i].r][q[i].c]/dist[q[rear].r][q[rear].c]);
       colorMode(RGB);
       fill(fillColor);
       noStroke();
...
public int onBit(int x, int k)
{
  return x | (1 << k);
}

public int offBit(int x, int k)
{
  return x & ~(1 << k);
}
  public void settings() {  size(1366, 768); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "Visualize_BFS" };
    if (passedArgs != null) {
